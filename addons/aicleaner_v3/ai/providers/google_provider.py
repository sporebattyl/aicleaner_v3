...
class GoogleProvider(BaseAIProvider):
    ...
    async def process_request(self, request: AIRequest) -> AIResponse:
        ...
        cache = get_ai_cache()
        cached_response = await cache.get("google", self.model_name, request.prompt, {
        ...
        if cached_response:
            self.logger.debug(f"Cache hit for Google request: {request.request_id}")
            return AIResponse(
        ...
        try:
            ...
            await cache.set("google", self.model_name, request.prompt, cache_data, {
                "max_tokens": self.max_tokens,
                "temperature": self.temperature,
                "has_image": bool(request.image_path or request.image_data)
            })
