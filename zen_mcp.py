#!/usr/bin/env python3
"""
Global Multi-API Key Zen MCP Client for Gemini Collaboration
...
async def zen_collaborate(prompt: str, api_keys: list = None) -> Dict[str, Any]:
...
Gemini, this is a test of our globally available zen MCP client that can be accessed from any directory in any Claude session.

Features being tested:
- Multi-API key support (3 keys, 4,050 total daily capacity)
- Intelligent model fallback (Pro → Flash → Flash-Lite)
- Thinking mode optimization per model
- Global availability across all sessions

Please confirm:
1. Which model and API key are responding
2. Verification that the global setup is working correctly
3. Readiness for AICleaner v3 collaborative prompt development

This confirms our permanent multi-session zen MCP capability.
"""
    
    result = await zen_collaborate(test_prompt)
    
    if result.get("success"):
        print(f"SUCCESS! Model: {result['model_used']} on {result['api_key_used']}")
